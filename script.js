...
document.querySelector('.check').addEventListener('click', function () { //Only called once event happens //#1create event listener
    ...
    } else if (guess !== secretNumber) { //#24 New refactored code
        if (score > 1) {
        //   document.querySelector('.message').textContent = guess > secretNumber ? '📈 Too High!' : '📉 Too Low!'; //Same as below
          displayMessage(guess > secretNumber ? '📈 Too High!' : '📉 Too Low!');
          score--
          document.querySelector('.score').textContent = score; //#13 update the score value
        } else {
          displayMessage('💥 You lost the game!'); //#15 make user aware that they lost the game because of too many attempts
